...
3. Run 'rake migrate' to setup initial database.
4. Add in user by running './script/console' (or 'ruby script\console' 
   under windows) and then entering the following command replacing
   values as appropriate.

   >> User.create!(:name => 'myusername', :password => 'mypass')

5. Run ./script/server (or 'ruby script\server' under windows) and point
